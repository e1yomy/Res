...
class UsuariosAPI {
	...
	function respuestaDatos($str) {
		...
		echo json_encode(array("Tabla" => $str));
    ...
    public function API(){
		...
		if (strlen($json_params) > 0 && $this->isValidJSON($json_params))
		{
			...
			case 'TipoUsuario'://Obtenemos la tabla de tipo de usuarios.
				$this->getTipoUsuario();
				break;     
			case 'Usuario'://Obtenemos la tabla de usuarios.		
				$this->getUsuarios();
				break;                
