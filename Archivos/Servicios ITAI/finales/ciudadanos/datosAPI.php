...
class DatosAPI {
	...
	function getDatos($Tabla)
	{
		...
		function acceso($tabla,$usuario,$contrasena)
	{
		try {
			$db = new DatosDB();  
			$response = $db->acceso($tabla,$usuario,$contrasena);    
			if ($response != null )
			{
				//$num = count($response);  
				$this->respuestaDatos($response);
	...
	function accesoSujetos($tabla,$usuario,$contrasena)
	{
		try {
			$db = new DatosDB();  
			$response = $db->accesoSujetos($tabla,$usuario,$contrasena);    
			if ($response != null )
			{
				//$num = count($response);  
				$this->respuestaDatos($response);
	...
	function listarRespuestaSolicitud($idAcceso)
	{
	try {
			$db = new DatosDB();  
			$response = $db->listarRespuestaSolicitud($idAcceso);    
			if ($response != null )
			{
				//$num = count($response);  
				//$this->respuestaDatos($response);
	...
	function listarSolicitudSujetos($tabla,$idSuj)
	{
		try {
			$db = new DatosDB();  
			$response = $db->listarSolicitudSujetos($tabla,$idSuj);    
			if ($response != null )
			{
				//$num = count($response);  
				$this->respuestaDatos($response);
    ...
    public function API()
	{
		...
		if (strlen($json_params) > 0 && $this->isValidJSON($json_params))
		{
			...
			switch ($decoded_params->funcion) {
				...
				break;
			case 'acceso' :
				$this->acceso($decoded_params->tabla,$decoded_params->usuario,$decoded_params->contrasena);
				break;
				case 'accesoSujetos' :
			...
			case 'registro' :
				...
				$this->listarRespuestaSolicitud($decoded_params->idAcceso);
				break;
				case 'misSolicitudes' :
				...
				break;
				case 'nuevoRecurso' :
				$this->nuevoRecurso($decoded_params->idUsuario, $decoded_params->folio, $decoded_params->idTipoDeEntrega,$decoded_params->idSujeto, $decoded_params->nombreSujeto, $decoded_params->causa, $decoded_params->motivo,$decoded_params->pruebas, $decoded_params->fecha);
				break;
