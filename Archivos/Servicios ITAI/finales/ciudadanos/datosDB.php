...
class DatosDB 
    ...
    public function listarSolicitudSujetos($tabla,$idSuj)
    {
    	$resultado = null;
		try {		
			$mydb = ConectarsePDO();
    ...
    public function listarRespuestaSolicitud($idAcceso)
    {
    	$resultado = null;
		try {		
			$mydb = ConectarsePDO();
			...
			if ($mydb!= null) 
			{				
				$query = "SELECT * FROM respuestaSolicitud where idAcceso = ". $idAcceso;
				$sql = $mydb->query($query);
				$resultado = $sql->fetch(PDO::FETCH_ASSOC);	
				//$resultado=$query;
			}
								...
								try {		
											...
											{			
													...
													if ($consulta->num_rows==0){
														...
														echo("update respuestaSolicitud set descripcion ='$descripcion', urlImagen ='$urlImagen', urlPDF ='$urlPDF' where idAcceso = $idAcceso");
